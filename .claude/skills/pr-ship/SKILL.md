...
WORKTREE_DIR="/tmp/agentspec-ship-${PR_NUMBER}"
...
- **Worktree path** — always `/tmp/agentspec-ship-<PR_NUMBER>` (distinct from pr-automation's `/tmp/agentspec-pr-*` and pr-verify's `/tmp/agentspec-verify-*`)
