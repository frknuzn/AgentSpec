...
**Worktree path:** always `/tmp/agentspec-verify-<PR_NUMBER>` — never use `/tmp/agentspec-pr-*` (reserved for pr-automation/pr-fix).
...
WORKTREE_DIR="/tmp/agentspec-verify-${PR_NUMBER}"
...
Create worktree at `/tmp/agentspec-verify-<PR_NUMBER>` (reuse if already created in Step 2):
...
补充测试建议 (N 个):
  ✅ [commit] src/common/utils/__tests__/token.verify.test.ts — 3 个用例覆盖 expiry 边界
  ⏭️ [skip]  src/common/utils/__tests__/token.verify.test.ts — 1 个集成用例，仅本地验证
...
在另一个终端中运行以下命令启动应用进行验证：
  ...
  cd /tmp/agentspec-verify-<PR_NUMBER> && bun run start
...
```bash
for dir in /tmp/agentspec-verify-*; do
...
```
 ...
 4. Impact analysis + test supplementation:
    a. Create worktree at /tmp/agentspec-verify-<PR> (reuse if from Step 2)
 ...
 7. Cleanup all /tmp/agentspec-verify-* worktrees → session summary (merged/auto-merge/rejected/skipped)
```
