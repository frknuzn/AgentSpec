...
```
~/.agentspec-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/AgentSpec/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/AgentSpec --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/AgentSpec/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/AgentSpec --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/AgentSpec --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.agentspec-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | iOfficeAI/AgentSpec | GitHub repository (owner/repo)                                      |
