 ...
 * parsing the electron-log file for [AgentSpec:ready] / [AgentSpec:init] /
 * [AgentSpec:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [AgentSpec:ready] marks
  ...
  // Parsed from [AgentSpec:init] marks
  initTotalMs: number;
  // Parsed from [AgentSpec:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'AgentSpec-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'AgentSpec', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'AgentSpec', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'AgentSpec', 'logs', `${today}.log`));
  }
...
// Matches: [AgentSpec:ready] <label> +<ms>ms
// Matches: [AgentSpec:init]  <label> +<ms>ms
// Matches: [AgentSpec:process] <label> +<ms>ms
const MARK_REGEX = /\[AgentSpec:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[AgentSpec] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[AgentSpec] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[AgentSpec] Showing main window')) marks.logs.showingMainWindow = true;
  }
...
async function launchApp(timeoutMs: number, withMemory: boolean): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      AGENTSPEC_DISABLE_AUTO_UPDATE: '1',
      AGENTSPEC_E2E_TEST: '1',
      AGENTSPEC_DISABLE_DEVTOOLS: '1',
      AGENTSPEC_CDP_PORT: '0',
      NODE_ENV: 'production',
    },
    timeout: timeoutMs,
  });
