...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentspec-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentspec-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      AGENTSPEC_EXTENSIONS_PATH: emptyExtensionsDir,
      AGENTSPEC_EXTENSION_STATES_FILE: extensionStatesFile,
