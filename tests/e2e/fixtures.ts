 ...
 *      (e.g. out/linux-unpacked/agentspec, out/mac-arm64/AgentSpec.app, out/win-unpacked/AgentSpec.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentspec-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/AgentSpec.exe  or  out/win-x64-unpacked/AgentSpec.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'AgentSpec.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/AgentSpec.app/Contents/MacOS/AgentSpec  or  out/mac/AgentSpec.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'AgentSpec');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/agentspec  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['agentspec', 'AgentSpec']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    AGENTSPEC_EXTENSIONS_PATH: process.env.AGENTSPEC_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    AGENTSPEC_EXTENSION_STATES_FILE: process.env.AGENTSPEC_EXTENSION_STATES_FILE || e2eStateFile,
