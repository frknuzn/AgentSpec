...
const testState = vi.hoisted(() => ({
  ...
  initialBuiltinServer: (): MockServer => ({
    ...
    transport: {
      ...
      env: {
        AGENTSPEC_IMG_PLATFORM: 'new-api',
        AGENTSPEC_IMG_BASE_URL: 'https://example.com',
        AGENTSPEC_IMG_API_KEY: 'key',
        AGENTSPEC_IMG_MODEL: 'grok-imagine-1.0',
      },
    },
  }),
  mockConfigGet: vi.fn(),
...
describe('ToolsModalContent image generation status refresh', () => {
  ...
  it('refreshes image generation agent status only after sync completes when enabling the builtin MCP server', async () => {
    ...
    expect(testState.mockCheckSingleServerInstallStatus).toHaveBeenCalledWith('agentspec-image-generation');
  });
