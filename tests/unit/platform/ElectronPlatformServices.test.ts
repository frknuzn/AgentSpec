...
describe('ElectronPlatformServices.paths.getLogsDir', () => {
  ...
  it('returns app.getPath("logs") when it succeeds', async () => {
    mockGetPath.mockImplementation((name: string) => {
      if (name === 'logs') return '/Users/test/Library/Logs/AgentSpec';
      if (name === 'userData') return '/Users/test/Library/Application Support/AgentSpec';
      return `/mock/${name}`;
    });
    ...
    expect(svc.paths.getLogsDir()).toBe('/Users/test/Library/Logs/AgentSpec');
  });

  it('falls back to userData/logs when app.getPath("logs") throws', async () => {
    const userData = '/Users/test/Library/Application Support/AgentSpec';
    mockGetPath.mockImplementation((name: string) => {
