...
describe('Step 6: loadBuiltinSessionMcpServers — builds servers list', () => {
  ...
  it('returns [teamServer] when teamMcpStdioConfig is set', async () => {
    ...
    expect(servers[0]).toMatchObject({ name: 'agentspec-team-abc', command: 'node' });
...
describe('Step 7a: createOrResumeSession — Codex vs non-Codex routing', () => {
  ...
  it('non-Codex resume: newSession options contain mcpServers array', async () => {
    ...
    expect(opts.mcpServers[0]).toMatchObject({ name: 'agentspec-team-abc' });
  });
...
describe('Step 7b PROOF-OF-FIX: Codex loadSession receives mcpServers (Task #1)', () => {
  ...
  it('PROOF-OF-FIX: loadSession receives team MCP server when teamMcpStdioConfig is set', async () => {
    ...
    // On fixed:   args[2] = [{name:'agentspec-team-abc',...}] (direct array) → PASSES
    expect(args[2]).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'agentspec-team-abc', command: 'node' })])
    );
...
describe('Step 8: Task #3 IPC mcpStatus events', () => {
  ...
  it('emits degraded when team config is missing but mcpServers ends up empty', async () => {
    ...
    const agent = createCodexAgent({
      teamMcpStdioConfig: { name: 'agentspec-team-abc', command: '', args: [], env: [] },
    });
    await callCreateOrResume(agent);
  ...
  it('event payload includes teamId derived from server name', async () => {
    ...
    // teamId extracted from 'agentspec-team-abc' → 'abc'
...
//
// How to verify "frontend can see MCP injection status" in a real AgentSpec env:
//
// HAPPY PATH:
//   1. Launch AgentSpec, open a Team with at least one ACP (claude/codex) agent
...
//
// DIAGNOSIS COMMANDS:
//   grep "Injecting team MCP server" ~/Library/Logs/AgentSpec/*.log
//   sqlite3 ~/Library/.../agentspec.db "SELECT extra FROM conversations WHERE id='<agentConvId>'"
