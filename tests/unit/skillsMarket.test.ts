 ...
 *
 * Tests the enable/disable flow for the agentspec-skills builtin skill:
...
const BUNDLED_SKILL_PATH = path.resolve(
  __dirname,
  '../../src/process/resources/skills/_builtin/agentspec-skills/SKILL.md'
);

// Mock Electron app and initStorage before importing AcpSkillManager
vi.mock('electron', () => ({ app: { setName: vi.fn(), getPath: () => '/tmp/agentspec-test' } }));
vi.mock('../../src/process/utils/initStorage', () => ({
  getSkillsDir: () => path.join('/tmp/agentspec-test', 'skills'),
  getAutoSkillsDir: () => path.join('/tmp/agentspec-test', 'skills', '_builtin'),
  getBuiltinSkillsCopyDir: () => path.join('/tmp/agentspec-test', 'builtin-skills'),
}));
...
describe('Skills Market - Bundled SKILL.md', () => {
  ...
  it('has valid frontmatter with name and description', async () => {
    ...
    expect(nameMatch![1].trim()).toBe('agentspec-skills');
  ...
  it('contains the curl command for fetching full SKILL.md', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('curl -s https://skills.agentspec.com/SKILL.md');
  });
  ...
  it('references the standard credentials path', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('~/.config/agentspec-skills');
  });
...
describe('Skills Market - Enable/Disable flow', () => {
  ...
  it('enable: creates agentspec-skills directory with SKILL.md', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'agentspec-skills');
    ...
    expect(written).toContain('name: agentspec-skills');
  });

  it('disable: removes agentspec-skills directory completely', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'agentspec-skills');
  ...
  it('disable: fs.rm with force does not throw if directory does not exist', async () => {
    const skillDir = path.join(tmpDir, '_builtin', 'agentspec-skills');
