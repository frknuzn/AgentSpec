...
const isE2ETestMode = process.env.AGENTSPEC_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.AGENTSPEC_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[AgentSpec] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[AgentSpec] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[AgentSpec] Creating main window...');
  ...
  console.log(`[AgentSpec] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[AgentSpec] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[AgentSpec] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[AgentSpec] Renderer did-finish-load');
      showWindow();
    });
  ...
  const disableAutoUpdater =
    process.env.AGENTSPEC_DISABLE_AUTO_UPDATE === '1' || process.env.AGENTSPEC_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[AgentSpec] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[AgentSpec] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[AgentSpec] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[AgentSpec] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[AgentSpec] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[AgentSpec] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[AgentSpec] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[AgentSpec] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[AgentSpec] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[AgentSpec] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[AgentSpec] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[AgentSpec] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[AgentSpec] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[AgentSpec:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[AgentSpec] Failed to restore zoom factor:', error);
...
// Register agentspec:// as the default protocol client
...
// macOS: handle agentspec:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[AgentSpec] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[AgentSpec] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[AgentSpec] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[AgentSpec] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[AgentSpec] quit (exitCode=${exitCode})`);
});
