...
export function initConversationBridge(
  ...
  const emitConversationListChanged = (
    ...
    ipcBridge.conversation.listChanged.emit({
      ...
      action,
      source: conversation.source || 'agentspec',
    });
  };
  ...
  ipcBridge.conversation.remove.provider(async ({ id }) => {
    try {
      ...
      // If source is not 'agentspec' (e.g., telegram), cleanup channel resources
      // 如果来源不是 agentspec（如 telegram），需要清理 channel 相关资源
      if (source && source !== 'agentspec') {
        try {
