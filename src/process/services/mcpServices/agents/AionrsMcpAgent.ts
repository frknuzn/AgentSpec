 ...
 * Maps to AgentSpec transport types (snake_case)
 */
...
/**
 * Map aionrs transport type (kebab-case) to AgentSpec transport type
 */
function toAgentSpecTransportType(aionrsType: AionrsTransportType): IMcpServerTransport['type'] {
  if (aionrsType === 'streamable-http') return 'streamable_http';
  return aionrsType;
}

/**
 * Map AgentSpec transport type to aionrs transport type (kebab-case)
 */
...
/**
 * Convert an aionrs server config entry to an AgentSpec IMcpServer
 */
function toMcpServer(name: string, config: AionrsServerConfig): IMcpServer {
  const transportType = toAgentSpecTransportType(config.transport);
  const now = Date.now();
...
/**
 * Convert an AgentSpec IMcpServer to an aionrs server config entry
 */
...
export class AionrsMcpAgent extends AbstractMcpAgent {
  ...
  getSupportedTransports(): string[] {
    // aionrs supports stdio, sse, streamable-http (mapped to streamable_http in AgentSpec)
