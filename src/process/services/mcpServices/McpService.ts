 ...
 * - 'agentspec': @office-ai/aioncli-core (AgentSpec 本地管理的 Gemini 实现)
 */
export class McpService {
  ...
  constructor() {
    this.agents = new Map([
      ...
      ['agentspec', new AionuiMcpAgent()], // AgentSpec 本地 @office-ai/aioncli-core
  ...
  private getAgentForConfig(agent: { backend: string; cliPath?: string }): IMcpProtocol | undefined {
    ...
    if (agent.backend === 'gemini' && !agent.cliPath) {
      return this.agents.get('agentspec');
    }
