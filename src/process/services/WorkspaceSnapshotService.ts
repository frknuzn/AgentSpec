...
export class WorkspaceSnapshotService {
  ...
  /**
   * Remove leftover `agentspec-snapshot-*` directories from previous sessions
  ...
  static async cleanupStaleSnapshots(): Promise<void> {
    ...
    const stale = entries.filter((name) => name.startsWith('agentspec-snapshot-'));
  ...
  private async createWorkingTreeSnapshot(workspacePath: string): Promise<string> {
    const gitdir = path.join(os.tmpdir(), `agentspec-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    await execFileAsync(
      ...
      [
        ...
        'user.name=AionUI',
        '-c',
        'user.email=snapshot@agentspec.local',
        'commit',
        '--allow-empty',
        '-m',
        'baseline',
