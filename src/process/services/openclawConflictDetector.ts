 ...
 * as AgentSpec Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with AgentSpec credentials
 */
export function detectLarkConflict(agentspecAppId: string): ConflictInfo | null {
  ...
  for (const [accountName, account] of Object.entries(feishu.accounts)) {
    if (account.enabled && account.appId === agentspecAppId) {
      console.warn(
        `[OpenClawConflictDetector] Lark conflict detected: OpenClaw account "${accountName}" uses same appId: ${agentspecAppId}`
      );
      return {
        platform: 'lark',
        openclawEnabled: true,
        credentialMatch: true,
        openclawCredential: agentspecAppId,
      };
    }
  }
...
/**
 * Check if OpenClaw Telegram channel conflicts with AgentSpec credentials
 */
export function detectTelegramConflict(agentspecBotToken: string): ConflictInfo | null {
  const config = readOpenClawConfig();
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as AgentSpec.`,
    ``,
    `This means messages are being handled by OpenClaw, not AgentSpec Channels.`,
    `Switching agents in AgentSpec will have no effect.`,
    ``,
