...
export async function resetPasswordCLI(username: string): Promise<void> {
  try {
    ...
    const dbPath = path.join(getDataPath(), 'agentspec.db');
    ...
    if (!hasUsersResult.data) {
      ...
      log.info('');
      log.info('Please run AgentSpec at least once to initialize the database:');
      log.info('  agentspec --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  agentspec --resetpass <username>');
      process.exit(1);
    }
