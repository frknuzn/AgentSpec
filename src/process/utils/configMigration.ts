...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.agentspec-config', 'agentspec-config.txt'),
      path.join(home, '.agentspec-config-dev', 'agentspec-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, 'AgentSpec', 'config', 'agentspec-config.txt'),
      path.join(appData, 'AgentSpec-Dev', 'config', 'agentspec-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', 'AgentSpec', 'config', 'agentspec-config.txt'),
    path.join(home, '.config', 'AgentSpec-Dev', 'config', 'agentspec-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[AgentSpec] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[AgentSpec] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[AgentSpec] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an agentspec-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[AgentSpec] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[AgentSpec] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[AgentSpec] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[AgentSpec] IMPORT_CONFIG_FROM failed:', error);
  }
}
