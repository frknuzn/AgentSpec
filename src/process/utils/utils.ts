...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'agentspec');
};
 ...
 * Release builds use ~/.agentspec; dev builds use ~/.agentspec-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.agentspec，Dev 模式使用 ~/.agentspec-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'agentspec');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.agentspec'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.agentspec-config; dev builds use ~/.agentspec-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.agentspec-config，Dev 模式使用 ~/.agentspec-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.agentspec-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[AgentSpec] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[AgentSpec] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[AgentSpec] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[AgentSpec] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
