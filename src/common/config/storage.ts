...
export interface IConfigStorageRefer {
  ...
  // Skills Market: whether the agentspec-skills builtin skill is enabled
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'agentspec' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 agentspec / Conversation source, defaults to agentspec */
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by AgentSpec (hide edit/delete in UI) */
  builtin?: boolean;
}
